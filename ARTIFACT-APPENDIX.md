...
This runs all three pipeline phases (train → attack → analysis) with 10 shadow models × 1 epoch.
Expected outputs:
...
- **Figure 8 requires multiple benchmarks.** The panel is composed from all 10 benchmarks
  (see `configs/figure_panels/`).  Reviewers can run only a subset to get a partial figure.
...
- **Figure 11 (appendix) requires a top-k 16 rerun and manual assembly.**  The default
  ...
  2 × 3 grid manually or via LaTeX.

- **Table 15 (appendix) requires the `cifar10_tl` benchmark.** Reproducing it requires running
