...
def setup_experiment(config):
    ...
    if config.get('experiment', {}).get('checkpoint_dir', 'none') == 'none':
        ...
        config['experiment']['checkpoint_dir'] = experiment_dir
    else:
...
def main():
    ...
    end_shadow_model_idx = config.get('training', {}).get('end_shadow_model_idx', config.get('training', {}).get('num_shadow_models', 1) - 1)
    logger.info(f"Training shadow models from index {start_shadow_model_idx} to {end_shadow_model_idx}...")

    for i in range(start_shadow_model_idx, end_shadow_model_idx+1):
        ...
        shadow_train_dataset = Subset(train_dataset, train_idxs[keep_indices[i]])
        shadow_train_dataset_eval = Subset(train_eval_dataset, train_idxs[keep_indices[i]])
        logger.info(f"Shadow train dataset size: {len(shadow_train_dataset)}")
        shadow_model = train_target_model(config, shadow_train_dataset, test_dataset, shadow_train_dataset_eval, device, shadow_model_dir, logger)
    ...
    # Aggregate and save shadow model utility metrics; can be used to compute Table 2 of the paper
    aggregate_shadow_metrics(
        experiment_dir,
