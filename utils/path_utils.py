...
def build_experiment_dir(config, created_at=None):
    """
    Resolve the experiment directory from config, creating a timestamped default when needed.

    Args:
        config: Configuration dictionary
        created_at: Optional datetime used for deterministic path generation in tests

    Returns:
        Path to the experiment directory
    """
    experiment_cfg = config.setdefault("experiment", {})
    checkpoint_dir = experiment_cfg.get("checkpoint_dir", "none")
    if checkpoint_dir not in {None, "", "none"}:
        return Path(checkpoint_dir)

    dataset = config.get("dataset", {}).get("name", "cifar10")
    model = config.get("model", {}).get("architecture", "resnet18")
    run_name = experiment_cfg.get("run_name", None)
    if run_name:
        return Path("experiments") / dataset / model / str(run_name)
    timestamp = (created_at or datetime.now()).strftime("%Y-%m-%d_%H%M")
    return Path("experiments") / dataset / model / timestamp
