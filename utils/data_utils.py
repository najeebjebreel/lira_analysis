...
def load_dataset(config):
    ...
    if dataset_name == 'cifar10':
        ...
        traindir = os.path.join(data_dir, dataset_name, "train")
        validdir = os.path.join(data_dir, dataset_name, "valid")
        testdir = os.path.join(data_dir, dataset_name, "test")
        train_dataset = ImageFolder(root=traindir)
        # valid_dataset = ImageFolder(root=validdir)
        test_dataset = ImageFolder(root=testdir)
        full_dataset = ConcatDataset([train_dataset, test_dataset])
        ...
        feature_path = os.path.join(data_dir, dataset_name, 'features.npy')
        label_path = os.path.join(data_dir, dataset_name, 'labels.npy')
        # Load and convert to appropriate dtype immediately
        full_features = torch.tensor(np.load(feature_path).astype(np.float32))
        full_label = torch.tensor(np.load(label_path).astype(np.int64)-1)
        full_dataset = TabularDataset(full_features, full_label)
        ...
        # Load and convert to appropriate dtype immediately
        full_features = torch.tensor(np.load(feature_path).astype(np.float32))
        full_label = torch.tensor(np.load(label_path).astype(np.int64))
        full_dataset = TabularDataset(full_features, full_label)
    else:
...
def load_dataset_for_mia_inference(config):
    ...
    if dataset_name == 'cifar10':
        ...
        validdir = os.path.join(data_dir, dataset_name, "valid")
        testdir = os.path.join(data_dir, dataset_name, "test")
        train_dataset = ImageFolder(root=traindir, transform=aug_transform)
        # valid_dataset = ImageFolder(root=validdir, transform=test_transform)
        test_dataset = ImageFolder(root=testdir, transform=aug_transform)
        full_dataset = ConcatDataset([train_dataset, test_dataset])
        train_label = np.array(train_dataset.targets)
        # valid_label = np.array(valid_dataset.targets)
        test_label = np.array(test_dataset.targets)
        full_label = np.concatenate((train_label, test_label), axis=0)
        ...
        label_path = os.path.join(data_dir, dataset_name, 'labels.npy')
        full_features = torch.tensor(np.load(feature_path).astype(np.float32))
        full_label = np.load(label_path).astype(np.int32)-1
        full_dataset = TabularDataset(full_features, full_label, transform=None)
        ...
        label_path = os.path.join(data_dir, dataset_name, 'labels.npy')
        full_features = torch.tensor(np.load(feature_path).astype(np.float32))
        full_label = np.load(label_path).astype(np.int32)
        full_dataset = TabularDataset(full_features, full_label, transform=None)
