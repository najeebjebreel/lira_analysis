    ...
    if train:
        ...
        for aug in aug_list:
            if aug == 'random_crop':
                ...
                padding_mode = config.get('training', {}).get('crop_padding_mode', 'constant')
                transform_ops.append(transforms.RandomCrop(input_size, padding=pad, padding_mode=padding_mode))
...
def get_keep_indices(dataset_size, num_shadow_models, pkeep, seed=0):
    ...
    if num_shadow_models <= 0:
        raise ValueError("training.num_shadow_models must be at least 1")

    rng = np.random.default_rng(seed)

    if num_shadow_models == 1:
        return (rng.uniform(0, 1, size=(1, dataset_size)) <= pkeep).astype(bool)
    
    # Vectorized implementation for multiple shadow models
    # Generate uniform random values for all models/samples at once
    keep_scores = rng.uniform(0, 1, size=(num_shadow_models, dataset_size))
    
    # Compute threshold based on pkeep
    threshold = min(num_shadow_models, max(0, int(pkeep * num_shadow_models)))
...
def _seed_dataloader_worker(worker_id, base_seed):
...
def load_dataset(config, mode='training'):
    ...
    Args:
        ...
        mode (str): Loading mode - 'training' or 'inference'
            - 'training': Returns datasets with train and test transforms, plus keep_indices and transforms separately
            - 'inference': Returns datasets with test transform only

    Returns:
        train_dataset, keep_indices, test_dataset, train_dataset_eval 
    """
    ...
    if mode == 'training':
        ...
        train_transform = test_transform = build_transforms(config, train=False)
    ...
    if dataset_name == 'cifar10':
        ...
        train_dataset_eval = CIFAR10(root=data_dir, train=True, download=True, transform=test_transform)
        ...
        train_dataset_eval = CIFAR100(root=data_dir, train=True, download=True, transform=test_transform)
        ...
        train_dataset_eval = GTSRB(root=data_dir, split='train', download=True, transform=test_transform)
    ...
    return train_dataset, keep_indices, test_dataset, train_dataset_eval
...
def create_data_loaders(train_dataset, test_dataset, train_dataset_eval, config):
    ...
    loader_kwargs = {
        ...
        'worker_init_fn': partial(_seed_dataloader_worker, base_seed=seed),
